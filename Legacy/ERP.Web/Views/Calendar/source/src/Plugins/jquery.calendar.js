...
//build month view
        ...
        function BuildWT(ht, dayarrs, events, dMax) {
            //1:
            ht.push("<tr>", "<th width=\"60\" rowspan=\"3\">ALL DAY</th>");
            ...
            if (option.url && option.url != "") {
                ...
                { name: "viewtype", value: option.view },
				 { name: "timezone", value: zone },
                    {name:"usrId",value:usr_id},
                       {name:"ccoId",value:ccoId}
                ];
                if (option.extParam) {
        ...
        function ConcatEvents(events, start, end) {
            ...
            if (events) {
                // 强制重置所有日历内容
                if (option.eventItems.length == 0 || true) {
                    //console.log(events);
        ...
        function dayshow(e, data) {
            ...
            if (data != null) {
                if (option.quickDeleteUrl != "" && data[8] == 1 && option.readonly != true) {
                    var csbuddle = '<div id="bbit-cs-buddle" style="z-index: 180; width: 400px;visibility:hidden;" class="bubble">' +
                        '<table class="bubble-table" cellSpacing="0" cellPadding="0">' +
                        '<tbody><tr>' +
                        '<td class="bubble-cell-side"><div id="tl1" class="bubble-corner"><div class="bubble-sprite bubble-tl"></div></div>' +
                        '<td class="bubble-cell-main"><div class="bubble-top"></div>' +
                        '<td class="bubble-cell-side"><div id="tr1" class="bubble-corner">' +
                        '<div class="bubble-sprite bubble-tr"></div></div>  ' +
                        '<tr><td class="bubble-mid" colSpan="3"><div style="overflow: hidden" id="bubbleContent1"><div><div></div>' +
                        '<div class="cb-root"><table class="cb-table" cellSpacing="0" cellPadding="0"><tbody><tr><td class="cb-value">' +
                        '<div class="textbox-fill-wrapper"><div class="textbox-fill-mid">' +
                        '<div id="bbit-cs-what" title="'
                    	+ i18n.xgcalendar.click_to_detail + '" class="textbox-fill-div lk" style="cursor:pointer;font-family: Calibri">' +
                        '</div>' +
                        '</div>' +
                        '</div>' +
                        '</td>' +
                        ...
                        '<tr><td class=cb-value><div id="bbit-cs-solink"></div></td></tr></tbody></table>' +
                        '<div class="bbit-cs-split"><input id="bbit-cs-id" type="hidden" value=""/>[ <span id="bbit-cs-delete" class="lk" style="font-family: Calibri">'
                    	+ i18n.xgcalendar.i_delete + '</span> ]&nbsp; <SPAN id="bbit-cs-editLink" class="lk" style="font-family: Calibri">'
...
//                            $('#bbit-cs-solink').append("<a id='alink_2_so' href='" + data[12] + "' target='_blank' style='font-weight: bold; color:blue'>" + data[11] + "</a>");
//                        } else {
//                            $('#bbit-cs-solink').empty("");
//                        }
//                        $('#bbit-cs-solink').click(function() {
//                        var win = window.open(data[12], '_blank');
//                        win.focus();
//                        });
...
//                            return false;
                    ...
                    var isDoneText = data[10] == "1" ? "<span style='color:red'>IS DONE </span>" : "";
                    var ts = $("#bbit-cs-buddle-timeshow").html(isDoneText +ss.join(""));
                    //console.log(data);
                    $("#bbit-cs-what").html(data[1]);
                    $("#bbit-cs-id").val(data[0]);
                    if (!IsNullOrEmpty(data[11])) {
                        $('#bbit-cs-solink').html("<a id='alink_2_so' href='" + data[12] + "' target='_blank' style='font-weight: bold; color:blue'>" + data[11] + "</a>");
                        $("#bbit-cs-solink").unbind("click");
                        $('#bbit-cs-solink').click(function() {
                            var win = window.open(data[12], '_blank');
                            win.focus();
                        });
                        ...
                        $.post(option.quickAddUrl, param, function (data) {
                            if (data) {
                                if (data.IsSuccess == true) {
                                    ...
                                    option.eventItems[tId][0] = (data.Data == undefined ? 0 : data.Data);
