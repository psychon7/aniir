...
import { mergeCredentials, getStorePath, listProfiles, setActiveProfile, readCredentials } from "../auth/credentials-store.js";
...
export async function runConnect({ argv = process.argv, env = process.env } = {}) {
  ...
  if (args.nonInteractive) {
    // Optional: clone credentials from another profile to avoid re-pasting secrets
    if (args.cloneFrom) {
      const base = await readCredentials(args.cloneFrom);
      const { updated_at, ...rest } = base ?? {};
      Object.assign(updates, rest);
      summary.saved.push(`cloned_from:${args.cloneFrom}`);
    }
