export function withConfigDefaults(input) {
  ...
  return {
    ...
    ai: {
      ...
      timeout_seconds: Number(config.ai?.timeout_seconds ?? 300),
...
export function validateConfig(config) {
  ...
  if (!Number.isFinite(config.ai?.timeout_seconds) || config.ai.timeout_seconds < 0) {
    throw new Error("ai.timeout_seconds must be a non-negative number (0 disables timeouts)");
  }
